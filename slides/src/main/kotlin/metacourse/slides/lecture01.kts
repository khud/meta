...
val doc = document(createPreamble("Введение")) {
    ...
    section("Введение") {
        frame("Что такое метавычисления?") {
            +"""Метавычисления - это область Computer Science, 
            которая занимается изучением методов преобразования программ"""
        }

        frame("Примеры метапрограмм") {
            itemize {
        ...
        frame("Компиляторы") {
            withImage("../assets/compilers.png") {
                +"""Теория построения компиляторов достаточно тщательно разработана, однако
                ориентирована на написание кода человеком, а не на автоматизацию процесса, за исключением
                некоторых совсем рутиных действий, таких как лексические и синтаксический анализ.
                """
            }
        }

        frame("Виртуальные машины") {
            +"""Виртуальные машины часто содержат в себе один и более компилятор,
            а также проводят сложные виды статических и динамических оптимизаций,
            например escape analysis.""".footnote("Escape analysis for Java: Theory and practice https://dl.acm.org/doi/10.1145/945885.945886")
        }

        frame("Статические анализаторы кода") {
            +"""Статические анализатора кода, также являются дальними родственниками
            метапрограмм, потому что используют схожие методы анализа, хотя не выполняют
            преобразования программ.
            """
        }
    }

    section("Частичные вычисления") {
        frame("Валентин Фёдорович Турчин") {
            withImage("../assets/turchin.jpg") {
                +"""${"Валентин Фёдорович Турчин".bold()} (14 февраля 1931, г. Подольск, Московская область — 7 апреля 2010, Нью-Йорк, США) — 
                советский и американский физик и кибернетик, создатель языка Рефал и новых направлений в программировании и 
                информатике, участник правозащитного движения в СССР, автор «самиздата», председатель Советский секции 
                «Международной амнистии».""".footnote(url("https://en.wikipedia.org/wiki/Valentin_Turchin"))
            }
        }

        frame("Академик А. П. Ершов") {
            withImage("../assets/ershov.png") {
                +"""${"Андрей Петрович Ершов".bold()} (19 апреля 1931, Москва — 8 декабря 1988, там же) — советский учёный, 
                один из пионеров теоретического и системного программирования, создатель Сибирской школы информатики, 
                академик АН СССР. Его работы оказали огромное влияние на формирование и развитие вычислительной техники 
                не только в СССР, но и во всём мире.
                
                Ершов - один из пионеров российской корпусной лингвистики; по его инициативе начал создаваться Машинный 
                фонд русского языка при Институте русского языка АН СССР.""".footnote(url("https://en.wikipedia.org/wiki/Andrey_Ershov"))
            }
        }
        ...
        frame("Применение частичных вычислений") {
            ...
            Возможная область применения частичных вычислений${cite(Bib.jones93)}:"""
            itemize {
    ...
    section("Специализатор и его свойства") {
        ...
        frame("Случай $ S = L_0 $") {
            ...
            $ spec(int, p)$ - это ${"результат компиляции".bold()} $ p $ с языка $ L $ в язык $ S $.
    ...
    section("Обобщения и частные случаи") {
        val C = symbol("C").mathcal()

        frame("Суперкомпиляция - С. М. Абрамов") {

            +"""${"Суперкомпиляция".bold()}—один из разделов метавычислений, совокупность понятий, 
            ...
            +"""Аргументами суперкомпилятора являются: $ p \in P $ — программа на языке $ L $ и $ $C $—класс, 
            обобщенные данные для $ p $."""

            +"""Таким образом, суперкомпилятор вычисляет программу $ p_{$C} $, которая является результатом 
            специализации $ p $ на класс $ $C $, то есть специальной версией программы $ p $ для случая, когда 
            входные данные имеют вид $ d \in $C $."""
        }

        frame("Специализация программ") {
            +"""Как следует из определения: специализация программ - это ${"частный случай".bold()} суперкомпиляции, 
            когда в качестве класс $ $C $ мы берем множество из одного элемента."""
        }

        frame("Виртуальные машины") {
            itemize {
                -"Какие преимущества и недостатки имеет виртуальная машина по сравнению с исполнением нативного кода?"
                -"Почему виртуальные машины так важны в контексте метавычислений?"
            }
        }
